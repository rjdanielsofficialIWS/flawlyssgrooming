...
export default function FurGalleryPage({ onNavigate }) {
  ...
  const galleryItems = useMemo(() => galleryCategories.flatMap((category) => {
    ...
    return images.map((image, index) => ({
      ...
      images: [image.src, ...(image.additionalSrcs || [])],
              ...
              <article className="gallery-result-card">
                <div className="gallery-result-media">
                  {item.images.map((src, photoIndex) => (
                    <img
                      src={src}
                      alt={photoIndex === 0 ? item.petName || item.caption || `Grooming gallery photo ${item.index}` : `${item.petName || 'Pet'} gallery photo ${photoIndex + 1}`}
                      loading="lazy"
                      key={`${item.key}-${photoIndex}`}
                    />
                  ))}
                  {item.images.length > 1 && <span className="gallery-photo-count">{item.images.length} photos · swipe</span>}
                </div>
